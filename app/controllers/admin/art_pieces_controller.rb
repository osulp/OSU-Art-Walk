class Admin::ArtPiecesController < AdminController
  ...
  def art_piece_params
    params.require(:art_piece).permit(:title, :creation_date, :size, :legal_info, :private, :contact_info, :description, :artist_comments, :displayed, :number, :percent_for_art, :building, art_piece_photo_ids:[], art_piece_photos_attributes: [:photo, :id, :_destroy], art_piece_building_attributes: [:building_id, :location, :art_piece_id, :position_num, :id], artist_ids: [], collection_ids:[], series_ids:[], medium_ids:[])
  end
end
